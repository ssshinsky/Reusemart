...
<div style="margin: 1rem 0; display: flex; gap: 1rem;">
    ...
    <button class="btn-action" id="editToggle">✏️ Edit Request Donasi</button>
</div>
    ...
    <div class="table-scroll-x">
        <table class="table-employees">
            ...
            <tbody id="pegawaiTableBody">
                @foreach($requestDonasis as $requestDonasi)
                    <tr>
                        <td class="center">{{ $requestDonasi->req }}</td>
                        <td style="{{ $penitip->status_penitip !== 'Active' ? 'color: #E53E3E; font-weight: bold;' : '' }}">{{ $penitip->nama_penitip }}</td>
                        <td class="action-cell" style="background-color:rgb(255, 245, 220)">
                            <a href="{{ route('requestDonasi.edit', $requestDonasi->id_pegawai) }}" class="edit-btn">✏️</a>
