                    ...
                    <div class="slider-container" id="slider-container">
                        @if ($barang->gambar && $barang->gambar->isNotEmpty())
                            @foreach ($barang->gambar as $gambar)
                                @php
                                    $imagePath = 'storage/gambar/' . $gambar->gambar_barang;
                                    $imageExists = file_exists(public_path($imagePath));
                                @endphp
                                @if ($imageExists)
                                    <img src="{{ asset($imagePath) }}" alt="{{ $barang->nama_barang ?? 'Gambar barang' }}" class="slide">
                                @else
                                    <p class="text-muted text-sm">Gambar tidak ditemukan: {{ $gambar->gambar_barang }}</p>
                                @endif
                            @endforeach
                        @else
                            <p class="text-muted text-sm">Tidak ada gambar untuk produk ini</p>
                        @endif
                    </div>
                    @if ($barang->gambar && $barang->gambar->isNotEmpty())
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css" integrity="sha512-SnH5WK+bZxgPHs44uWixMB8O BEbS4S8J7O1zLbjW3fW5z5B5z5f5z5B5z5f5z5B5z5f5z5B5z5f5z5B5z5f5z5A==" crossorigin="anonymous" referrerpolicy="no-referrer" />
<style>
    :root {
    ...
    .star-empty .star-background { fill: #d1d5db; }
    .penitip-badge {
        display: inline-flex;
        align-items: center;
        gap: 0.5rem;
    ...
    .penitip-name { font-weight: 600; text-transform: capitalize; }
    ...
    @media (max-width: 768px) {
        ...
        .slider-container, .slide { height: 200px; }
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/js/all.min.js" integrity="sha512-u3fPA7V8qQmhBPNT8b6NV0i6a3fhrNEUjEBRxxS8S49u3V1ZAa+5rN0N3D+Hg3vKKH2KHC3lEb+YQ==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function showSlide(index) {
            if (!slides.length || !sliderContainer) {
                console.warn('No slides or slider container found, skipping slide update');
                return;
            }
        ...
        function moveSlide(direction) {
            ...
            console.log('Moved slide, direction:', direction, 'new index:', slideIndex);
        }

        function currentSlide(index) {
            slideIndex = index - 1;
            showSlide(slideIndex);
            console.log('Selected slide index:', slideIndex);
        }
        ...
        if (slides.length > 0) {
            ...
            console.warn('No slides available, auto-slide disabled');
        }
        ...
        if (addToCartForm) {
            addToCartForm.addEventListener('submit', async function(e) {
                ...
                try {
                    ...
                    successModal.show();
                    console.log('Cart add success:', response.data.message);
                } catch (error) {
